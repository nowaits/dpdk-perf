...
application_send_attach(udp_echo_main_t *utm)
{
    ...
    bmp->options[APP_OPTIONS_ADD_SEGMENT_SIZE] = 1 << 20;
    bmp->options[APP_OPTIONS_SEGMENT_SIZE] = 128 << 10;
...
server_handle_fifo_event_rx(udp_echo_main_t *utm, u32 session_index)
{
    ...
    int rx_bytes = app_recv_dgram_raw(rx_fifo, utm->rx_buf, max_transfer, &session->transport, 1, 0);

    if (rx_bytes <= 0) {
        return;
    }
    app_send_dgram(session, utm->rx_buf, rx_bytes, SVM_Q_WAIT);
#else
    vppcom_data_segments_t ds = {};
    int rx_bytes = svm_fifo_segments(rx_fifo, (svm_fifo_segment_t *) ds);
    svm_fifo_unset_event(rx_fifo);
    svm_fifo_segments_free(rx_fifo, (svm_fifo_segment_t *) ds);
#endif
    RECORD(rx_bytes - sizeof(session_dgram_hdr_t), 1);
    return;
#endif
