        ...
        for (i = 0; i < num_ev; i++) {
            ...
            if (vsm->cfg.proto == VPPCOM_PROTO_UDP) {
                if (EPOLLIN & wrk->wait_events[i].events) {
                    vppcom_endpt_t _ed = {0}, *ed = &_ed;
                    vppcom_data_segments_t ds = {};
                    char buf[1000] = {0};
#if 1
                    int rx_bytes = vppcom_session_read_segments(wrk->listen_fd, ds);
                    if (1) {
                        session_dgram_hdr_t *p = (session_dgram_hdr_t *)buf;
                        char *data = buf + sizeof(*p);
                        vppcom_data_segment_copy(buf, ds, sizeof(buf));
                    }

                    vppcom_session_free_segments(wrk->listen_fd, ds);

#else
                    int rx_bytes = vppcom_session_recvfrom(wrk->listen_fd, buf, sizeof(buf), 0,  NULL);
#endif
                    status.total += rx_bytes - sizeof(session_dgram_hdr_t);
                    status.cur_pps ++;
                    if (clib_cpu_time_now() - status.last >= os_cpu_clock_frequency()) {
                        status.pps = status.cur_pps;
                        status.cur_pps = 0;
                        status.speed = status.total * 8;
                        status.total = 0;
                        status.last = clib_cpu_time_now();
#define _(data, tag1, tag2) \
    \
    if (data > 1024 * 1024 * 1024) { \
        fprintf(stderr, tag1"%ldG"tag2, data>> 30); \
    } else if (data > 1024 * 1024) { \
        fprintf(stderr, tag1"%ldM"tag2, data>> 20); \
    }  else if (data > 1024) { \
        fprintf(stderr, tag1"%ldK"tag2, data>> 10); \
    } else { \
        fprintf(stderr, tag1"%ld"tag2, data); \
    }

                        _(status.speed, "speed: ", "b/s - ")
                        _(status.pps, "pps: ", "b/s\n")
#undef _
                    }
